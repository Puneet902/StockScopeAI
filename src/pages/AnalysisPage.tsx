...
export function AnalysisPage() {
  ...
  const analyzeStock = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8000/analyze?symbol=${encodeURIComponent(cleanSymbol)}&tf=${timeframe}`
      );
  ...
  const fetchFundamentals = async (stockSymbol: string) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/fundamentals?symbol=${encodeURIComponent(stockSymbol)}`
      );
              ...
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[75%] rounded-2xl px-3.5 py-2.5 select-text ${
                      msg.role === 'user' 
                        ? theme === 'dark'
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            <div className="max-w-7xl mx-auto p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full border rounded-lg pl-10 pr-4 py-3 placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors ${
                        theme === 'dark'
                  ...
                  <select
                    ...
                    className={`border rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors ${
                      theme === 'dark'
                        ? 'bg-white/5 border-white/10 text-white'
                        : 'bg-gray-100 border-gray-200 text-gray-900'
                    }`}
                  ...
                  <button
                    ...
                    disabled={loading}
                    className={`border font-medium rounded-lg px-6 py-3 flex items-center justify-center gap-2 transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${
                      theme === 'dark'
              ...
              {!result && !error && !loading && (
                <div className={`backdrop-blur-md border rounded-2xl p-8 mb-6 ${
                  theme === 'dark'
                    ? 'bg-neutral-900/80 border-white/10'
                    : 'bg-white/90 border-gray-200'
                }`}>
                    ...
                    <div className={`backdrop-blur-md border rounded-2xl p-6 ${
                      ...
                      <div className="flex flex-col h-full justify-center">
                        ...
                        <div className={`text-xl font-bold ${
                          (result.price_change_percent || 0) >= 0 ? 'text-green-400' : 'text-red-400'
                        }`}>
